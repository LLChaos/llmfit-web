"""Context scorer — evaluates model context length relative to 32K benchmark.

Weight: 15% of total recommendation score.

Formula:
    base_score = min(100, (model_context / 32768) * 100)
    vram_ratio = min(1.0, vram_available / recommended_vram_gb)
    final_score = base_score * vram_ratio

The 32K (32768) benchmark reflects the current mainstream model ceiling.
Models at 128K still cap at 100. The VRAM ratio reduces the score when
the GPU may not have enough memory to fully utilize the maximum context.
"""
...
def score_context(model: dict[str, object], vram_available: float) -> float:
    ...
    base_score = min(100.0, (context_length / 32768.0) * 100.0)  # type: ignore[operator]
