...
import { TradeoffBars } from "@/components/tradeoff-bar";
...
export function ModelCard({ model, rank, sortBy, onClick }: ModelCardProps) {
  ...
  const reasonText =
    model.reason ? t(`reason.${model.reason}` as TranslationKey) : "";

  return (
    <Card
      className="w-full cursor-pointer transition-all duration-200 hover:-translate-y-0.5"
      onClick={() => onClick(model)}
    >
      <CardContent className="flex flex-col gap-3 p-4">
        {/* Top row: rank + info + score ring */}
        ...
        {/* Trade-off bars */}
        <TradeoffBars
          quality={model.scores.quality}
          speed={model.scores.speed}
          compatibility={model.scores.compatibility}
          context={model.scores.context}
        />
      </CardContent>
    </Card>
  );
